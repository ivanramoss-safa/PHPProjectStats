...
final class HomeController extends AbstractController
{
    ...
    public function index(
        ...
        $nextDayStr = date('Y-m-d', strtotime($dateString . ' +1 day'));
        $prevDayStr = date('Y-m-d', strtotime($dateString . ' -1 day'));
        $datesToFetch = [$nextDayStr, $dateString, $prevDayStr];
        $lesionesRaw = [];
        foreach ($datesToFetch as $d) {
            $resp = $apiService->getCumulativeInjuries(['league' => 140, 'date' => $d]);
            $lesionesRaw = array_merge($lesionesRaw, $resp['response'] ?? []);
        }
        $seen = [];
        $lesiones = [];
        foreach ($lesionesRaw as $inj) {
            $pid = $inj['player']['id'] ?? 0;
            if (!isset($seen[$pid])) {
                $seen[$pid] = true;
                $lesiones[] = $inj;
        ...
        $lesiones = array_slice($lesiones, 0, 10);
